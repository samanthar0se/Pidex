...
The worker imports only Pi's documented public SDK surfaces. It uses Pi's public resource loader so the pinned runtime preserves normal global and project resource discovery, including extensions, skills, prompts, settings, models, and credentials. Project-local executable resources remain deny-by-default until Pi's project-trust flow receives an explicit answer through Pidex.
