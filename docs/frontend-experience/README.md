...
- **FX-DISC-04A:** Project each row's cue through four independent channels — leading state rail, icon, explicit text label, and semantic color — so no state depends on color or animation alone. Motion is exclusive to `working`; stillness with stronger contrast means it is the user's turn. Successful completion and blocking are distinct treatments, never one generic unread mark. Idle is the absence of exceptional state: no rail, no icon, no label. Selection and status stay separate concepts, so a selected row keeps its rail, icon, and label. The indicator slot is fixed width so row text never shifts between states, and reduced motion drops animation while the rail, icon, and label still identify the state.
- **FX-DISC-04B:** The Host derives the discovery row detail line — a current-activity phrase while `working`, the blocking reason while `needs response`, and plain recency otherwise — from exact Run, Interaction, and Timeline facts. The Client never invents recency or activity copy from Timeline heuristics.
