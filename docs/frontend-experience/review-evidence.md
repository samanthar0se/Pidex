...
**Status: delivery verified for issue #145 / PRD #137.** The five canonical journeys are implemented by delivery issues #138–#144 and exercised at the Client-store, React composition, generated-asset/real-Host, and performance-budget seams. Responsive and visual evidence remains bounded as documented below; no unsupported Codex-mobile claim or blocking discrepancy remains.
...
| Resolve Interactions | none/one/several; four kinds; deadline order; pending and terminal states; competing-command/Stop/worker-loss races | Interaction Variant B, including competing-command simulation; mobile `interactions`; `FX-INT-*` |
...
`D`, `C`, and `M` mean desktop, constrained width, and narrow mobile. “All” requires applicable coverage at all three classes. Delivery links identify the owning tickets and focused executable evidence; `npm test` is the complete real-Host/product gate.

### Shared delivery evidence

Every requirement below is covered by the [#138–#145 delivery campaign](https://github.com/samanthar0se/Pidex/issues/137) and the focused [`production-client-store`](../../test/production-client-store.test.ts), [`keyboard`](../../test/production-client-keyboard.test.ts), and [`real Host/assets`](../../test/production-client-assets.product.test.ts) suites.
...
|---|---|---|---|---|---|---|---|---|
| `FX-FOUND-01`–`02` | State inventory; package decision | All; exact fact vocabulary | Non-visual authority rule; `CONTEXT.md` glossary | All | Pidex language intentionally replaces Codex terms | Recognition uses exact facts, never aggregate Session state | Verified | [Shared delivery evidence](#shared-delivery-evidence) |
| `FX-STATE-01`–`05` | State inventory; fidelity boundary | Discover, work, resolve, recover; attention/read/current/target/draft deltas | Discovery and Composer complete scenario menus | All | Calm Codex rows; Pidex adds persistent actionable and trust exceptions | Recognition items 2–6; exact controls keyboard-reachable | Verified | [Shared delivery evidence](#shared-delivery-evidence) |
| `FX-DISC-01`–`04` | Discovery decision | Discover; populated/empty, attention × read, expansion/order | [Discovery desktop](evidence/discovery-desktop.png), [constrained](evidence/discovery-constrained.png) | D/C; drawer equivalent M | Direct Codex shell/row rhythm; Projects + Chats is Pidex hierarchy | `Ctrl+K`, row arrows, visible focus; one-frame selection feedback | Verified | [Shared delivery evidence](#shared-delivery-evidence) |
| `FX-DISC-04A`–`04B` | Session state sidebar handoff | Discover; working/blocked/review/idle and their transitions | [Session state prototype](../references/session-status-presentation-comparison.html) and [handoff](../references/session-state-sidebar-design-handoff.md); desktop workbench capture | All | Rail + icon + label replaces the shared dot; Host-derived detail line is a Pidex addition | Four non-color channels; motion only while working; reduced motion retains state; fixed indicator slot | Verified | [`session-attention-summary`](../../test/session-attention-summary.product.test.ts), [`client.visual`](../../test/visual/client.visual.test.ts) |
| `FX-DISC-05`–`06` | Discovery decision | Discover; search/no result/archive/restore/routing/no wake | Discovery Variant D scenario menu | All | Search stays in calm rail; Pidex Archive lifecycle is not a Codex claim | Search clear/return and route focus contract; current Session recognizable | Verified | [Shared delivery evidence](#shared-delivery-evidence) |
| `FX-DISC-07`–`10` | Discovery decision | Create; scope, capability, draft, rejection/partial success/uncertainty | [New Session](evidence/new-session-desktop.png) and Variant D state menu | All | Codex-like centered blank Composer; Pidex separates create and Run authority | Submit/newline/IME, validation focus, draft continuity, command-phase recognition | Verified | [Shared delivery evidence](#shared-delivery-evidence) |
| `FX-TL-01`–`05` | Timeline decision | Work/resolve; normal, streaming, tool, terminal, Interaction facts | Immutable PiRemote renderer and fixture; visual capture omitted because existing implementation is canonical | All | Closest available Codex-like narrative; Pidex removes split pane and keeps exact facts | Disclosure keyboard semantics; restrained motion; work/action/outcome recognition | Verified | [Shared delivery evidence](#shared-delivery-evidence) |
| `FX-TL-06`–`07` | Timeline decision | Work; older page, errors, mutable tail, reading upward | Non-visual behavioral contract grounded in PiRemote thread primitives and Host cursor paging | All | Pidex cursor authority, not Codex backend behavior | Anchor preservation, explicit fallback, no forced focus; stable loading | Verified | [Shared delivery evidence](#shared-delivery-evidence) |
| `FX-COMP-01`–`04` | Composer decision | Create/work; quiet, executing, steering, settled | [Composer executing](evidence/composer-executing-desktop.png), Variant D states | All | Codex-like persistent Composer; implicit steering and exact Stop are Pidex semantics | Enter/Shift+Enter/IME; one-frame acknowledgment; exact target recognizable | Verified | [Shared delivery evidence](#shared-delivery-evidence) |
| `FX-COMP-05`–`06` | Composer decision | Work/recover; held, offline, pending/rejected/uncertain | Composer Variant D `held`, `offline`, `uncertain` | All | Pidex authority deviation | Release/Cancel tab order; command phase and next action recognizable | Verified | [Shared delivery evidence](#shared-delivery-evidence) |
| `FX-INT-01`–`03` | Interaction decision | Resolve; one/several, draft empty/non-empty, takeover/return | [Interaction stack](evidence/interactions-desktop.png), [mobile](evidence/mobile-interactions.png) | All | Codex control tokens only; layout is intentionally Pidex due evidence gap | Deterministic takeover/return focus; short interruptible swap motion | Verified | [Shared delivery evidence](#shared-delivery-evidence) |
| `FX-INT-04`–`07` | Interaction decision | Resolve; pending, competing command, deadline, withdrawal, Stop, privacy | Variant B competing-command simulation; non-visual privacy rule | All | Pidex exact Interaction model | Pending row focus, next-request focus, exact target/result/next action recognizable | Verified | [Shared delivery evidence](#shared-delivery-evidence) |
| `FX-RESP-01`–`03` | Quality gate; mobile decision | All; long/dense/short/keyboard/safe-area/orientation stress | Desktop, constrained, and narrow capture set plus complete prototype state menus | All | Desktop parity only; constrained adaptation preserves hierarchy | No hidden primary action; anchor and focus preserved under pressure | Verified | [Shared delivery evidence](#shared-delivery-evidence) |
| `FX-RESP-04`–`07` | Mobile decision | Discover/work/resolve/recover on narrow Client | [Mobile working](evidence/mobile-working.png), [mobile Interactions](evidence/mobile-interactions.png), prototype offline/drawer/held scenarios | M | Explicit Pidex adaptation; no Codex-mobile claim | Touch-safe visible Stop/action/trust; dock continuity; functional drawer motion | Verified | [Shared delivery evidence](#shared-delivery-evidence) |
| `FX-TRUST-01`–`06` | Fidelity boundary; state inventory; mobile decision; forward-rendering compatibility decision | Recover; all connection and uncertain-receipt deltas; historical Session opened in a current compatible Client | Discovery offline, Composer offline/uncertain, mobile offline prototype state; non-visual projection compatibility contract | All | Quiet current Codex shell; Pidex escalates non-current authority and does not maintain a legacy presentation branch | Connection does not steal focus; stale/time/disabled reason/receipt phase recognizable; historical Session remains readable | Verified | [Shared delivery evidence](#shared-delivery-evidence) |
| `FX-KEY-01`–`05` | Quality gate; consolidated contract | All; complete traversal and focus restoration | Non-visual normative keyboard map, exercised by each journey fixture during delivery | All | Product correctness, not a Codex or WCAG claim | Contract is complete; no Stop shortcut avoids ambiguous exact target | Verified | [Shared delivery evidence](#shared-delivery-evidence) |
| `FX-VIS-01`–`02` | Codex baseline; fidelity boundary | Discover/create/work representative desktop states | Desktop capture set and named first-party references in research note | D/C | Review geometry, type, density, color, border, control, icon, hierarchy; every deviation named | State-recognition overlay remains compatible with calm rhythm | Verified | [Shared delivery evidence](#shared-delivery-evidence) |
| `FX-QUAL-01`–`04` | Quality gate | All; transitions, waits, loading, recognition | Prototype transitions and complete recognition checklist; numerical budgets from `performance-budgets.ts` | All | Restrained Codex rhythm; Pidex state clarity has priority | Motion interruptible; input one frame; p95 budgets explicit; seven recognition questions | Verified | [Shared delivery evidence](#shared-delivery-evidence) |
| `FX-ARCH-01`–`04` | Implementation strategy; Timeline decision | All; production seams | Non-visual architecture decision; PiRemote implementation validates assistant-ui/store seam | All | Stack enables fidelity without importing Codex authority | Feature interfaces are test seams; exact authority remains inspectable | Verified | [Shared delivery evidence](#shared-delivery-evidence) |
| `FX-ARCH-05`–`06` | Implementation strategy | All; delivery evidence and PWA lifecycle | Non-visual layered test/build contract | All | Screenshot review, not brittle pixel diffs | Vitest/RTL/Playwright/Host suites plus owned service-worker invariants | Verified | [Shared delivery evidence](#shared-delivery-evidence) |
...
### Presentation timing of a settling Run (`FX-STATE-02`, `FX-DISC-04A`, `FX-FOUND-02`)

Read status follows presentation, so the Client claims read-through only for the Timeline revision a View has rendered, and only after that tail has been quiet and visible for `tailPresentationDwellMs`. Arriving content is not read content: a streaming Run advances the Timeline revision many times a second, and a Run that settles as its reader leaves keeps its unread milestone. A reader who stays presents the tail once the stream stops, which is the exact fact the Host records.

The settling row is the one accepted timing deviation from `FX-FOUND-02`. A Run settles as two exact facts — the attention summary, then the unread milestone — both of which reach discovery before the reader's own presentation can answer either. The row therefore holds `working` for `reviewHoldMs` and then presents whatever is true, so a reader already watching sees `working → idle` instead of a Review flash. The hold never applies to `needs response`, which blocks the reader now, and it delays presentation only: it never invents a state, and the underlying Session, Run, and read facts stay exact throughout. Covered by `test/session-timeline.test.ts` and `test/production-client-store.test.ts`.
...
- [x] Every matrix row links the #138–#145 delivery campaign and executable Client/real-Host evidence.

## Delivery gate record

Issue #145 reviewed the production Client beside the named desktop references and checked-in desktop, constrained, and mobile captures. Geometry, typography, spacing, density, neutral color, separators, rounded controls, Lucide icon weight, and narrative hierarchy retain the accepted quiet-workbench direction. Pidex terminology, Projects + Chats, exact-target Stop, Interaction takeover, trust treatment, and the absence of suggestion cards/split review remain required semantic or explicit-scope deviations. Interaction, disconnected, and mobile layouts remain named evidence gaps; mobile is a Pidex adaptation and makes no Codex-parity claim.

The executable delivery chain is:

- **Discover/resume and create/start:** #138–#140; `test/production-client-store.test.ts` and `test/production-client-assets.product.test.ts`.
- **Work/settle:** #141–#142; `test/session-timeline.test.ts`, `test/live-session-timeline.product.test.ts`, and the exact Composer tests.
- **Resolve Interactions:** #143; `test/interaction-control.test.ts`, `test/interaction-races.product.test.ts`, and Client-store privacy/race coverage.
- **Recover continuity:** #144; Client-store recovery plus Host-loss and worker-loss product suites.
- **Responsive, keyboard, recognition, visual, and quality gate:** #145; `test/production-client-keyboard.test.ts`, responsive declarations in `apps/client/src/style.css`, the essential captures above, generated production assets, and `test/performance-budgets.product.test.ts`.

External packaged p95 measurements remain release artifacts under `docs/performance-evidence.md`; the executable gate validates the unchanged numerical budgets and fails closed on missing samples rather than inferring release performance from unit-test wall time.
