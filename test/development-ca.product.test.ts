...
test("setup creates and then reuses one constrained profile Development CA", async () => {
  ...
  const options = { profileRoot: profile, trustCurrentUserCertificate: (path: string) => trusted.push(path) };
  try {
    const first = setupDevelopmentCa(options);
    const firstKey = await readFile(join(profile, "Pidex", "Development CA", "pidex-development-ca-key.pem"));
    const second = setupDevelopmentCa(options);
    const certificate = new X509Certificate(await readFile(first.certificatePath));
    ...
    assert.ok(!firstKey.includes(Buffer.from("CERTIFICATE")), "only the public path is trusted");
  } finally { await rm(root, { recursive: true, force: true }); }
});

test("existing damaged state is never repaired or rotated", async () => {
  const root = await mkdtemp(join(tmpdir(), "pidex-development-ca-"));
  const options = { profileRoot: join(root, "profile"), trustCurrentUserCertificate: () => {} };
  try {
    const first = setupDevelopmentCa(options);
    await unlink(first.certificatePath);
    assert.throws(() => setupDevelopmentCa(options), /Development CA unusable.*dev:ca:reset.*dev:ca:setup/);
...
test("corrupt and profile-invalid state fail closed", async () => {
  ...
  try {
    ...
    assert.throws(() => setupDevelopmentCa(options), /Development CA unusable/);
    assert.throws(() => setupDevelopmentCa({ ...options, profileRoot: "relative" }), /LocalAppData profile.*invalid/);
  } finally { await rm(root, { recursive: true, force: true }); }
});
  ...
  try {
    assert.throws(
      () => setupDevelopmentCa({
        ...
        runOpenSsl: () => { throw Object.assign(new Error("spawn openssl"), { code: "ENOENT" }); },
      }),
      (error: unknown) => error instanceof DevelopmentCaPrerequisiteError && /openssl version/.test(error.message),
    );
  } finally { await rm(root, { recursive: true, force: true }); }
});
