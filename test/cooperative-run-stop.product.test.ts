...
test("stop is exact, retryable, and atomically cancels continuation without touching a sibling Session", async () => {
  ...
  try {
    ...
    assert.equal(siblingRun.kind, "accepted");
    ...
    assert.equal(
      store.loadInteraction(interaction.interactionId).state,
      "withdrawn",
    );
    assert.equal(
    ...
    const settled = store.settleRun(
      first.run.runId,
      "cancelled",
      "partial work retained; no rollback",
      null,
      7,
    );
    assert.equal(settled.run.state, "cancelled");
    assert.equal(store.runs(session.sessionId)[0]?.state, "cancelled");
