...
test("Host reconstructs its selector from sealed Authority generations", async () => {
  ...
  try {
    ...
    const recovered = await startHost({ dataDir, port: 0, adapters: adaptersFor("deterministic") });
    try {
    ...
    const again = await startHost({ dataDir, port: 0, adapters: adaptersFor("deterministic") });
    try { assert.equal(again.status().hostId, status.hostId); } finally { await again.close(); }
