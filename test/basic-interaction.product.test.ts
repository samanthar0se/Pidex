...
test("all basic Interaction kinds validate and settle only through their exact worker", async () => {
  ...
  const applied: PiInteractionResult[] = [];
  adapters.pi.execute = async request => {
    const onInteraction = request.onInteraction;
    assert.ok(onInteraction);
    for (const interaction of requests) {
      applied.push(await onInteraction(interaction));
    }
  ...
  try {
    ...
    for (;;) {
      ...
      if (
        scoped.type === "scope.reset" &&
        scoped.barrier.scope.kind === "session"
      ) {
        break;
      }
    }
    ...
    const answers: Array<string | boolean> = ["b", true, "Ada", "text"];
    for (let index = 0; index < requests.length; index++) {
      ...
      if (terminal.type === "interaction.change") {
        assert.equal(
          terminal.interaction.state,
          index === 1 ? "dismissed" : "responded",
        );
      }
    }
    while ((await nextControlMessage(socket)).type !== "run.completed") {
      // Wait for settlement after the final Interaction resolves.
    }
    assert.deepEqual(applied, [
      { dismissed: false, value: "b" },
      { dismissed: true },
      { dismissed: false, value: "Ada" },
      { dismissed: false, value: "text" },
    ]);
    socket.close();
  } finally {
    await host.close();
...
async function nextOpenInteraction(socket: WebSocket): Promise<Interaction> {
  for (;;) {
    ...
    if (
      message.type === "interaction.change" &&
      message.interaction.state === "open"
    ) {
