...
test("reidentification gates recovery, exports first, and establishes a wholly new trust basis", async () => {
  ...
  try {
    ...
    const db = new DatabaseSync(authority);
    db.exec("CREATE TABLE devices(id TEXT); INSERT INTO devices VALUES ('phone'); CREATE TABLE recovery(event TEXT, old_host TEXT, new_host TEXT, epoch TEXT)");
    db.close();
    const calls: string[] = [];
    ...
    const recovery = new HostReidentification({
      root,
      authority,
      oldIdentity: { hostId: "old-host", origin: "https://old.pidex.local" },
      localhost: () => true,
      trust: () => ({ hostKey, caKey, portableIdentity }),
      verifyAuthority: () => authorityValid,
      createVerifiedExport: async () => {
        calls.push("export");
        if (exportFails) throw new Error("export-interrupted");
        return { id: "backup-1", encrypted: true, verified: true };
      },
      issueIdentity: () => {
        calls.push("identity");
        return { hostId: "new-host", origin: "https://new.pidex.local", ca: "new-private-ca" };
      },
      reconcile: (copy, epoch) => {
        calls.push("reconcile");
        copy.exec("DELETE FROM devices");
        copy.prepare("INSERT INTO recovery VALUES ('cryptographic-continuity-ended', 'old-host', 'new-host', ?)").run(epoch);
      },
      activateTrust: identity => calls.push(`trust:${identity.origin}`),
    });

    assert.throws(() => recovery.preview({ localhost: false }), /host-local-recovery-required/);
    portableIdentity = true;
    assert.throws(() => recovery.preview({ localhost: true }), /portable-identity-restore-required/);
    portableIdentity = false;
    hostKey = true;
    caKey = false;
    assert.match(recovery.preview({ localhost: true }).confirmation, /END CONTINUITY/);
    hostKey = false;
    caKey = true;
    const preview = recovery.preview({ localhost: true });
    ...
    exportFails = true;
    await assert.rejects(() => recovery.commit(preview.confirmation), /export-interrupted/);
    assert.deepEqual(calls, ["export"]);
    exportFails = false;
    const result = await recovery.commit(preview.confirmation);
    assert.equal(result.identity.origin, "https://new.pidex.local");
    assert.deepEqual(calls, ["export", "export", "identity", "reconcile", "trust:https://new.pidex.local"]);
    const restored = new DatabaseSync(result.authority, { readOnly: true });
    assert.equal(restored.prepare("SELECT count(*) AS count FROM devices").get()!.count, 0);
    assert.equal(restored.prepare("SELECT event FROM recovery").get()!.event, "cryptographic-continuity-ended");
    restored.close();
    const pairing = new DatabaseSync(result.authority);
    pairing.exec("INSERT INTO devices VALUES ('freshly-paired-phone')");
    assert.equal(pairing.prepare("SELECT count(*) AS count FROM devices").get()!.count, 1);
    pairing.close();
...
test("corrupt authority permits only an encrypted, exact, non-restorable evidence package", async () => {
  ...
  try {
    ...
    const recovery = new HostReidentification({
      root, authority: damaged,
      oldIdentity: { hostId: "old", origin: "https://old" },
      localhost: () => true,
      trust: () => ({ hostKey: false, caKey: false, portableIdentity: false }),
      verifyAuthority: () => false,
      createVerifiedExport: async () => ({ id: "unused", encrypted: true, verified: true }),
      issueIdentity: () => ({ hostId: "new", origin: "https://new", ca: "ca" }),
      reconcile: () => {}, activateTrust: () => {},
    });
    assert.throws(() => recovery.preview({ localhost: true }), /authority-reference-closure-failed/);
    const evidence = await recovery.exportEvidence({
      localhost: true, passphrase: "secret", files: [damaged, intact],
    ...
    assert.notEqual(createHash("sha256").update(await readFile(evidence.path)).digest("hex"), "");
    assert.doesNotMatch((await readFile(evidence.path)).toString(), /damaged rows|intact store/);
  } finally { await rm(root, { recursive: true, force: true }); }
});
