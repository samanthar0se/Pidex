...
test("maintenance retains reachable ancestry and uses durable independent proofs for orphan cleanup", async () => {
  ...
  try {
    ...
    const accepted = store.submitRun("device", {
      commandId: "run", sessionId: parent.sessionId, prompt: "hello", requiredCapability: "run.submit",
    }, 2);
    ...
    store.forkSession(parent.sessionId, point.entryId, undefined, undefined, "child", "checkpoint", 4);
    ...
    store.retainObjectReference("recovery-manifest", "snapshot-1", `sha256:${protectedDigest}`);

    const outside = join(root, "outside");
    await mkdir(outside);
    await writeFile(join(outside, "sentinel"), "safe");
    await symlink(outside, join(root, "blobs", "escape"));

    const first = store.runMaintenance(10);
    assert.deepEqual(first.quarantined.sort(), [
      `sha256:${orphanDigest}`,
      `sha256:${racedDigest}`,
    ].sort());
    // A manifest published after the first proof wins over collection.
    store.retainObjectReference("rollback", "generation-1", `sha256:${racedDigest}`);
    ...
    assert.equal(await readFile(join(root, "blobs", racedDigest), "utf8"), "raced");
    assert.equal(store.projection().sessions.some(s => s.sessionId === parent.sessionId), true);
    assert.equal(store.projection().sessions.some(s => s.sessionId === "child"), true);
  } finally {
    store?.close();
