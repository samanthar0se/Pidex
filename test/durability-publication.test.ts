...
function assertPublicationCollision(
  run: () => void,
): PublicationCollisionError {
  try {
    run();
  } catch (error) {
    assert.ok(error instanceof PublicationCollisionError);
    return error;
  }

  assert.fail("Expected publication to collide");
}

test("immutable publication is validated, flushed, published, and idempotent", () =>
  withFixture(root => {
    ...
    assert.equal(
      publishImmutableFile(request, adapter).outcome,
      "already-published",
    );
    ...
    assert.equal(
      readdirSync(root).some(name => name.endsWith(".stage")),
      false,
    );
  }));
...
test("immutable collision preserves candidate evidence without overwriting authority", () =>
  withFixture(root => {
    ...
    publishImmutableFile({
      target,
      materialize: writeCandidate("first"),
      validate: () => true,
    });

    const collision = assertPublicationCollision(() =>
      publishImmutableFile({
        target,
        materialize: writeCandidate("second"),
        validate: () => true,
      }),
    );
...
test("invalid and flush-failed candidates are cleaned without publication", () =>
  withFixture(root => {
    const target = join(root, "object");
    assert.throws(
      () =>
        publishImmutableFile({
          target,
          materialize: writeCandidate("bad"),
          validate: () => false,
        }),
      /validation failed/,
    );
    assert.throws(
      () =>
        publishImmutableFile(
          {
            target,
            materialize: writeCandidate("good"),
            validate: () => true,
          },
          createDeterministicPublicationAdapter({
            failAt: "regular-files-flushed",
          }),
        ),
      /Injected/,
    );
...
test(
  "publication rejects a materializer that leaves its writer open",
...
test("validated trees flush regular files and rebuildable files replace atomically", () =>
  withFixture(root => {
    ...
    publishValidatedTree({
      target: tree,
      materialize(stage) {
        mkdirSync(join(stage, "nested"));
        writeFileSync(join(stage, "nested", "manifest"), "sealed");
      },
      validate(path) {
        return (
          readFileSync(join(path, "nested", "manifest"), "utf8") === "sealed"
        );
      },
    });
    ...
    replaceRebuildableFile({
      target: selector,
      materialize: writeCandidate("one"),
      validate: () => true,
    });
    replaceRebuildableFile({
      target: selector,
      materialize: writeCandidate("two"),
      validate: path => readFileSync(path, "utf8").length > 0,
    });
    assert.equal(readFileSync(selector, "utf8"), "two");
  }));

test("Windows records unsupported directory flush honestly", () =>
  withFixture(root => {
    const steps: PublicationStep[] = [];
    publishImmutableFile(
      {
