...
const manifest: ModuleManifest = {
  id,
  version: "1.0.0",
  hostCompatibility: { major: 1 },
  resourceKinds: [{ name: kind, scope: "workspace" }],
  protocolFamilies: [`${id}/process-v1`],
  capabilities: [capability],
  storage: [
    {
      name: `${id}/authority`,
      sqliteNamespace: "pidex_workspace__authority",
      blobKinds: [`${id}/log`],
      migrations: [{ from: 0, to: 1, migrate() {} }],
      checkIntegrity() {},
      enumerateBackupReferences: () => [],
      applyRetention() {},
      validateRestore() {},
    },
  ],
  diagnostics: [`${id}/process-health`],
  lifecycleServices: [`${id}/process-jobs`],
  ui: uiSlots.map((slot, index) => ({
    id: `${id}/ui-${index}`,
    slot,
    capability,
...
test("bundled modules use the kernel authority, storage, worker, UI, and future workspace seams", async () => {
  ...
  kernel.register(manifest, {
    commands: {
      [`${id}/start`]: payload => ({ accepted: payload }),
    },
  });
  ...
  const command = {
    ...
    capability,
    observedRevision: 3,
    payload: { workspaceId: "workspace-1", locator: "C:\\mutable" },
  };
  const accepted = await kernel.dispatchWorkerRequest(
    {
