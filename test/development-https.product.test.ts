...
test("checkouts reuse one CA and replace only invalid disposable typed-SAN leaves", async () => {
  ...
  try {
    const setup = setupDevelopmentCa({
      profileRoot,
    ...
    const first = provisionDevelopmentCertificate({
      ...
      aliases: ["10.2.3.4", "2001:db8::1", "dev.example"],
    });
    const restarted = provisionDevelopmentCertificate({
      profileRoot,
      dataDir: checkoutA,
      hostname: "dev.example",
      aliases: ["2001:db8::1", "10.2.3.4"],
    });
    const other = provisionDevelopmentCertificate({
      profileRoot,
      dataDir: checkoutB,
    ...
    for (const leaf of [first, other]) {
      assert.equal(
        new X509Certificate(leaf.ca).fingerprint256,
        setup.fingerprint,
      );
      const certificate = new X509Certificate(leaf.cert);
      assert.equal(certificate.checkIssued(new X509Certificate(leaf.ca)), true);
      assert.ok(certificate.subjectAltName);
      assert.match(
        certificate.subjectAltName,
        /DNS:dev\.example|DNS:other\.example/,
      );
      assert.match(certificate.subjectAltName, /IP Address:127\.0\.0\.1/);
      assert.match(
        certificate.subjectAltName,
        /IP Address:0:0:0:0:0:0:0:1|IP Address:::1/,
      );
    }
    assert.deepEqual(
      (await readdir(join(checkoutA, "development-tls"))).sort(),
      ["leaf-key.pem", "leaf.pem"],
    );

    await writeFile(
      join(checkoutA, "development-tls", "leaf-key.pem"),
      "mismatched key",
    );
    const replaced = provisionDevelopmentCertificate({
      profileRoot,
      dataDir: checkoutA,
      hostname: "dev.example",
      aliases: ["10.2.3.4", "2001:db8::1"],
    });
...
test("development startup serves real HTTPS only after profile setup", async () => {
  ...
  const profileRoot = join(root, "profile");
  const provisioner: HostCertificateProvisioner = ({ dataDir, hostname }) =>
    provisionDevelopmentCertificate({
      profileRoot,
      dataDir,
      hostname,
