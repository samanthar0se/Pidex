...
test("durable follow-ups execute once in order and are held after an abnormal predecessor", async () => {
  ...
  const pending: Array<{ prompt: string; resolve(value: PiExecuteResult): void; reject(error: Error): void }> = [];
  const base = adaptersFor("deterministic");
  const pi: PiAdapter = {
    ...base.pi,
    execute: request => new Promise((resolve, reject) => pending.push({ prompt: request.prompt, resolve, reject })),
  };
  const host = await startHost({ dataDir, port: 0, authorization: "device", adapters: { ...base, pi } });
  try {
    ...
    const submit = (type: "run.submit" | "run.follow-up", commandId: string, prompt: string) =>
      socket.send(JSON.stringify({ type, commandId, sessionId, prompt, requiredCapability: type }));
    ...
    pending.shift()!.resolve({ text: "done one", checkpoint: "cp1" });
    await nextControlMessage(socket);
    await new Promise(resolve => setImmediate(resolve));
    assert.deepEqual(pending.map(item => item.prompt), ["two"]);
    pending.shift()!.reject(new Error("failed two"));
    await nextControlMessage(socket);
    ...
    if (scope.type !== "scope.reset" || !("runs" in scope.snapshot)) throw new Error("missing runs");
    assert.deepEqual(scope.snapshot.runs?.map(run => run.state), ["completed", "failed", "held"]);

    const held = scope.snapshot.runs![2]!;
    socket.send(JSON.stringify({ type: "run.release", commandId: "release", runId: held.runId }));
    assert.equal((await nextControlMessage(socket)).type, "command.outcome");
    await new Promise(resolve => setImmediate(resolve));
    assert.deepEqual(pending.map(item => item.prompt), ["three"]);
    assert.equal(two.type === "command.outcome" && two.outcome, "accepted");
    socket.close();
  } finally {
    await host.close();
...
test("an undispatched follow-up survives restart held and can be cancelled without dispatch", async () => {
  ...
  try {
    ...
    const first = store.submitRun("device", { commandId: "one", sessionId: session.sessionId, prompt: "one", requiredCapability: "run.submit" }, 2);
    const next = store.submitRun("device", { commandId: "two", sessionId: session.sessionId, prompt: "two", requiredCapability: "run.follow-up" }, 3);
