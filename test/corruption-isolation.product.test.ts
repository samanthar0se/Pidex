...
test("scrubbing repairs only an exact proven copy and isolates unprovable damage", async () => {
  ...
  try {
    ...
    const scrubber = new CorruptionScrubber(root, [
      {
        id: "blob-one", kind: "blob", path: "live/one", digest: hash("exact"),
        scope: { kind: "session", id: "session-one" },
        copies: [{ path: "backup/one", provenance: "offline-backup-7", digest: hash("exact") }],
      },
      {
        id: "artifact-two", kind: "pi-checkpoint", path: "live/two", digest: hash("wanted"),
        scope: { kind: "session", id: "session-two" }, copies: [],
      },
    ]);
    ...
    assert.match(await readFile(join(root, "corruption-diagnostics.jsonl"), "utf8"), /offline-backup-7/);
...
test("global authority corruption enters capability-gated local recovery", async () => {
  ...
  try {
    ...
    const scrubber = new CorruptionScrubber(root, [{
      id: "host-identity", kind: "host-identity", path: "identity", digest: hash("valid"),
      scope: { kind: "global" }, copies: [],
    }], { recoverySecret: "signed-cli-secret" });
    scrubber.scrub({ now: 1, byteBudget: Infinity });
    assert.deepEqual(scrubber.availability(), {
      mode: "recovery", lanService: false, mdns: false, pairedDevicesAccepted: false,
    });
    ...
    assert.equal(scrubber.authorizeRecoveryLaunch("192.168.1.2", capability, 10), false);
  } finally { await rm(root, { recursive: true, force: true }); }
});
