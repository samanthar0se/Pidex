...
export async function startHost(options: HostOptions): Promise<StartedHost> {
  ...
  async function handleDirectoryBrowse(
    ...
    try {
      if (!admittedCapabilityBasisByClient.get(client)?.has("directory.browse@1")) {
        throw new Error("capability-unavailable");
      }
      const paths = message.parentToken
        ? await childDirectories(requireDirectoryToken(tokens, message.parentToken))
        : directoryRoots();
  ...
  async function handleProjectAdd(
    ...
    try {
      if (!admittedCapabilityBasisByClient.get(client)?.has("project.add-from-directory@1")) {
        throw new Error("capability-unavailable");
      }
      admitDiscretionaryWrite();
